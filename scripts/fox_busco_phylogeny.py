...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    per_gene_fastas = emit_gene_fastas(gene_index, samples, min_required, per_gene_dir)
    if not per_gene_fastas:
        LOGGER.error("No BUSCO genes met the minimum sample threshold of %d", min_required)
        return 1

    aligned_fastas = align_genes(per_gene_fastas, align_dir, args)
    if not aligned_fastas:
        LOGGER.error("Alignment stage produced zero files")
        return 1
